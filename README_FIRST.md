...
Google Drive OAuth is used as the document repository for normal Gmail / personal Drive accounts. Create a Google Cloud OAuth Web client and add `http://localhost:3000/api/google-drive/callback` as an authorized redirect URI, then set `GOOGLE_OAUTH_CLIENT_ID`, `GOOGLE_OAUTH_CLIENT_SECRET`, and `GOOGLE_OAUTH_COOKIE_SECRET`. Service-account Drive variables are still supported for Workspace Shared Drives, but normal My Drive uploads require the OAuth connection popup in the app.
...
The AI Portfolio Analyst is optional. Set `OPENAI_API_KEY` only on the server or in Vercel environment settings. It uses the OpenAI Responses API with structured output and defaults to `gpt-5.5`; set `OPENAI_AI_MODEL` to override the model and `OPENAI_AI_REASONING_EFFORT` to tune analysis depth. The app sends a reduced portfolio snapshot containing summarized amounts and holding labels, not profile details, account names, notes or documents.
...
5. Open Recommendations or Insights and click Generate AI Review after configuring `OPENAI_API_KEY`.
...
- Do not expose `OPENAI_API_KEY` in the frontend or name it with the `NEXT_PUBLIC_` prefix.
