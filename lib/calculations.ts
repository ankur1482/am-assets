...
function projectedFixedIncomeValue(r:any,periodYears:number){
  ...
  if(isPf(r)){
    let balance=initial,ratePerMonth=rate/12,monthsCount=Math.max(0,Math.round(periodYears*12));
    for(let i=0;i<monthsCount;i++)balance=(balance+monthly)*(1+ratePerMonth);
    return balance;
  }
  if(isAnnualAccount(r)){
    let balance=initial,fullYears=Math.floor(Math.max(0,periodYears)),partial=Math.max(0,periodYears-fullYears);
